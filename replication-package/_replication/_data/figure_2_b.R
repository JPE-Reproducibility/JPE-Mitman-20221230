...
lm_all           = lm(as.numeric(abs(hp_errors_med)) ~  wealth_1 + wealth_2 + wealth_3 + wealth_4 + wealth_5  + male + college + participate + age  + agesq  + factor(qdate), data=data_sce)
robust_all       = sqrt(diag(vcovHC(lm_all, type = "HC1")))


cond_mean      = rep(NA,n)
tmp            = data_sce[data_sce$wealth_1 == 1,]
cond_mean[1]   = mean(abs(tmp$hp_errors_med),na.rm=TRUE)
tmp            = data_sce[data_sce$wealth_2 == 1,]
cond_mean[2]   = mean(abs(tmp$hp_errors_med),na.rm=TRUE)
tmp            = data_sce[data_sce$wealth_3 == 1,]
cond_mean[3]   = mean(abs(tmp$hp_errors_med),na.rm=TRUE)
tmp            = data_sce[data_sce$wealth_4 == 1,]
cond_mean[4]   = mean(abs(tmp$hp_errors_med),na.rm=TRUE)
tmp            = data_sce[data_sce$wealth_5 == 1,]
cond_mean[5]   = mean(abs(tmp$hp_errors_med),na.rm=TRUE)
tmp            = data_sce[data_sce$wealth_6 == 1,]
cond_mean[6]   = mean(abs(tmp$hp_errors_med),na.rm=TRUE)
...
pdf(file="_figures/figure_2b_lhs.pdf")
barCenters = barplot(betas,   main="",xlab="Wealth Percentile", ylab="Absolute Error", border="darkorange", col="darkorange",  density=56, names.arg=c("0-10", "10-20", "20-40", "40-60", "60-80","80-100"), ylim = c(0,1.20))
abline(h=0)
...
lm_all           = lm(as.numeric(abs(hp_iqr_unc)) ~  wealth_1 + wealth_2 + wealth_3 + wealth_4 + wealth_5  + male + college + participate + age  + agesq  + factor(qdate), data=data_sce)
robust_all       = sqrt(diag(vcovHC(lm_all, type = "HC1")))
...
pdf(file="_figures/figure_2b_rhs.pdf")
