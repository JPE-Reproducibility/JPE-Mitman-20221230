...
for j=1:ns*npk
    for zcc = 1:nz
        cahp(:,zcc,j)    = rebate*Kgrid(exotok(j))*wtax+sav_grid*(1+r_mat(zcc,j)-delta-wtax)/(1-probdeath) ...
                            + ix(exotos(j))*w_mat(zcc,j) * ( iu(exotos(j))*(1-taxu_mat(zcc,j)) + (1-iu(exotos(j)))*(1-taxe_mat(zcc,j)) );
    end
end
...
for m=1:maxIter
    t1start = tic;
    cp = zeros(nk,npzp,ns*npk,nz);
    c_new = zeros(nA,npzp,ns*npk);
    cp_info = zeros(nk,ns*npk,nz);
...
% fully internalizing the uncertainty over capital tomorrow?
    ...
    for j=1:ns*npk
        ...
        % doing things linearly with the c code
            for pzc=1:npzp
                ...
                cp(:,pzc,j,zcc)  = interp1(Agrid,c0(:,pzc,j),cahp(:,zcc,j),'pchip','extrap');
                % if (cp(2:end,pzc,j,zcc)-cp(1:end-1,pzc,j,zcc)<0)>0
                %     disp([pzc,j,zcc])
                %     keyboard
                % end
            ...
            cp_info(:,j,zcc)     = interp1(Agrid,c0(:,1+(zcc-1)*(npzp-1),j),cahp(:,zcc,j)-nu,'pchip','extrap');
                % if cp_info(:,j,zcc)<0
                %     keyboard
                % end
    ...
    Emupzz = zeros(nk,npzp,ns*npk,nz);
    ...
    % do by info, the posterior is either 1 or npzp depending on zcc.
    for j=1:ns*npk
        for zcc=1:nz
    ...
    parfor j=1:ns*npk
        for zc=1:nz
            for zcc=1:nz
               ...
               % if sum((Emup(2:end,pzc,j)-Emup(1:end-1,pzc,j))>0)>0
                    % disp([pzc,j])
                   %disp
 %keyboard
                % end
    ...
    % test = zeros(nz,ns*npk);
    % for j=1:ns*npk
    %     for zc=1:nz
    %         for zcc=1:nz
    %             indz=zcc+(zc-1)*nz;
    %             test(zc,j) = test(zc,j)+ones(1,npk*ns)*squeeze(pi_exo_full(j,:,indz))';
    %         end
    %     end
    % end
...
%     keyboard
    ...
    for j=1:ns*npk
        for zc=1:npzp
            ...
            c_new(:,zc,j) = (Agrid>cah_today(1,zc,j)).*interp1([bmin; cah_today(:,zc,j)],[0; c_s(:,zc,j)],Agrid,'pchip','extrap')...
