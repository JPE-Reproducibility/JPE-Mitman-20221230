...
params.nsigK = parIn.nsigK;
nsigK = params.nsigK;
params.sigKmin=parIn.sigKmin;
sigKmin=params.sigKmin;
params.sigKmax=parIn.sigKmax;
...
% Variance of capital points
params.sigKgrid = linspace(sigKmin,sigKmax,nsigK);
% params.sigKgrid =logspace(log10(params.sigKmin),log10(params.sigKmax),params.nsigK);
params.sigKgrid=params.sigKgrid';
...
params.sigK_trans   = zeros(npk,npk,nsigK,nsigK,nz);
...
sigKdist0=parIn.sigKdist0all*ones(params.N,1);
sigKbar=parIn.sigKdist0all;
...
%     end
% end
if(cNa~=nA || cNpz~=npzp || cNaggK~=npk*ns*nsigK)
    cpol = 0.1*repmat(Agrid',[1 npzp npk*ns*nsigK]);
end
cNa=size(ipol,1);
cNpz=size(ipol,2);
cNaggK=size(ipol,3);

if(cNa~=nk || cNpz~=npz || cNaggK~=npk*ns*nsigK)
    if(params.exinfo>0)
        ipol = params.exinfo*ones(nk,npz,npk*ns*nsigK);
    elseif cNpz==npz && cNaggK==npk*ns*nsigK
        ipolnew=ones(nk,npz,npk*ns*nsigK);
	    for jj=1:npz
   		     for ii=1:npk*ns*nsigK
        ...
        ipol = 0.16*ones(nk,npz,npk*ns*nsigK);
    end
else
...
if(params.exinfo>0)
    ...
    if(size(X,2)==nz)
        ...
        cNaggK=size(X,4);
        if(cNa~=nA || cNpz~=npzp || cNaggK~=npk*ns*nsigK)
            X=log(repmat(reshape(cpol,[nA 1 npzp npk*ns*nsigK]),[1 nz 1 1]));
        end
    else
        cNa=size(X,1);
        cNpz=size(X,2);
...
while(diff_a > tol && iter<parIn.maxIterLOM)
    ...
    % For experience utility we need to the true evolution of K
    for zc=1:nz
        for kc=1:npk
    ...
    Kmap    = griddedInterpolant(log(params.Kgrid), (1:params.npk)', 'linear', 'nearest');

    params.a0=a0;
    params.a1=a1;
    [KK,VV,PP] = ndgrid(log(params.Kgrid(:)),params.sigKgrid(:),params.zpost(:));
    [logkp,sigKp] = mv_update(KK,VV,PP,a0,a1);
    iFloat = Kmap(logkp);               
    iL = floor(iFloat);                
    iU = min(iL+1, npk);                
    omegaK = iU - iFloat;              
    ...
    if params.nsigK > 1
        sigKmap = griddedInterpolant(params.sigKgrid, (1:params.nsigK)', 'linear', 'nearest');
        
        jFloat = sigKmap(sigKp);
        ...
        wUL = (1-omegaK) .* omegaV;
        wLU = omegaK .* (1-omegaV);
        wUU = (1-omegaK) .* (1-omegaV);


    else

        [logkp,sigKp] = mv_update_nov(KK,VV,PP,a0,a1);
        Kmap    = griddedInterpolant(params.Kgrid, (1:params.npk)', 'linear', 'nearest');

        iFloat = Kmap(exp(logkp));               
        iL = floor(iFloat);                
        iU = min(iL+1, npk);                
        omegaK = iU - iFloat; 
        jL = ones(size(iFloat));   % all mass at j=1
        jU = jL;                   % same cell
        omegaV = ones(size(iFloat));

        % reduces to linear in K only (accumulate to the single j=1 column)
        wLL = omegaK;              % to (iL, j=1)
        wUL = 1 - omegaK;          % to (iU, j=1)
    ...
    ksigkprob=zeros(npk*nsigK,npk*nsigK,npzp);

    for sigK=1:nsigK
        for kc=1:npk
            for pzc=1:npzp
                % ksigkprob(idx(kc,sigK,npk),idx(iL(kc,sigK,pzc),jL(kc,sigK,pzc),npk),pzc)=wLL(kc,sigK,pzc);
                % ksigkprob(idx(kc,sigK,npk),idx(iU(kc,sigK,pzc),jL(kc,sigK,pzc),npk),pzc)=wUL(kc,sigK,pzc);
                % ksigkprob(idx(kc,sigK,npk),idx(iL(kc,sigK,pzc),jU(kc,sigK,pzc),npk),pzc)=wLU(kc,sigK,pzc);
                % ksigkprob(idx(kc,sigK,npk),idx(iU(kc,sigK,pzc),jU(kc,sigK,pzc),npk),pzc)=wUU(kc,sigK,pzc);
                row = idx(kc, sigK, npk);
                ksigkprob(row, idx(iL(kc,sigK,pzc), jL(kc,sigK,pzc), npk), pzc) = ...
                  ksigkprob(row, idx(iL(kc,sigK,pzc), jL(kc,sigK,pzc), npk), pzc) + wLL(kc,sigK,pzc);
                
                ksigkprob(row, idx(iU(kc,sigK,pzc), jL(kc,sigK,pzc), npk), pzc) = ...
                  ksigkprob(row, idx(iU(kc,sigK,pzc), jL(kc,sigK,pzc), npk), pzc) + wUL(kc,sigK,pzc);
                
                if params.nsigK > 1
                    ksigkprob(row, idx(iL(kc,sigK,pzc), jU(kc,sigK,pzc), npk), pzc) = ...
                      ksigkprob(row, idx(iL(kc,sigK,pzc), jU(kc,sigK,pzc), npk), pzc) + wLU(kc,sigK,pzc);
                    
                    ksigkprob(row, idx(iU(kc,sigK,pzc), jU(kc,sigK,pzc), npk), pzc) = ...
                      ksigkprob(row, idx(iU(kc,sigK,pzc), jU(kc,sigK,pzc), npk), pzc) + wUU(kc,sigK,pzc);
                end        
            end
        end
    end
    for pzc = 1:npzp
      s = sum(ksigkprob(:,:,pzc), 2);
      assert(all(abs(s - 1) < 1e-12), 'ksigkprob row not stochastic for some pzc');
    end
    params.ksigkprob=ksigkprob;


    % we know c(m',eps',pK',sigK')
    % there's a unique mapping (p_z,pK,sigK)|->(p_z,pK',sigK') (this is
    % params.ksigprob)
    % int (c(m'(pkk,z),pK',sigK')dF(pkk)
    % F = truncate normal with pK, sigK
    % to do the int use probkp, wKp_grid

    probkp = zeros(npk,npk*nsigK*npzp);

    L = a0(1)/(1-a1(1));
    U = a0(2)/(1-a1(2));
    kmin = floor(Kmap(L));
    kmax = ceil(Kmap(U));
    logKgrid = log(params.Kgrid);
    edges = [logKgrid(kmin); 0.5*(logKgrid(kmin:kmax-1)+logKgrid(kmin+1:kmax));logKgrid(kmax)];
    for idc=1:npzp*npk*nsigK
        mkp=logkp(idc);
        vkp=sigKp(idc);
        sigkp=sqrt(vkp);
        if(sigkp<1e-10)
            probkp(iL(idc),idc)=omegaK(idc);
            probkp(iU(idc),idc)=1-omegaK(idc);
        else
            alphaval = (L - mkp)./sigkp;
            betaval  = (U - mkp)./sigkp;
            kappaval = normcdf(betaval) - normcdf(alphaval);
            zz=(edges-mkp)/sigkp;
            cumF = normcdf(zz);
            probkp(kmin:kmax,idc)=diff(cumF)/kappaval;
            probkp(:,idc) = probkp(:,idc)/sum(probkp(:,idc));
        end
    end
    params.probkp=probkp;




    % wKp_grid: (realized K' bin) × (kp_bel) × (sigKp_bel)
    wKp_grid = zeros(npk, npk, nsigK);
    
    % use the same truncated-lognormal machinery you used for probkp_today,
    % but center it at the *belief* node:
    if nsigK>1
        logK   = log(params.Kgrid);
    else
        logK   = params.Kgrid;
    end
    L = a0(1)/(1-a1(1));  U = a0(2)/(1-a1(2));
    kmin = floor(Kmap(L));  kmax = ceil(Kmap(U));
    edges = [logK(kmin); 0.5*(logK(kmin:kmax-1)+logK(kmin+1:kmax)); logK(kmax)];
    
    for kpB = 1:npk
        mkp = logK(kpB);
        for sKB = 1:nsigK
            vkp = params.sigKgrid(sKB);  sigkp = sqrt(max(vkp,0));
            if sigkp < 1e-10
                % deterministic to nearest bin
                iF = Kmap(mkp); iL = max(1,floor(iF)); iU = min(npk,iL+1);
                wL = iU - iF;  wU = 1 - wL;
                w = zeros(npk,1);  w(iL)=wL; w(iU)=wU;
            else
                alpha=(L - mkp)/sigkp; beta=(U - mkp)/sigkp; kap = max(normcdf(beta)-normcdf(alpha),1e-15);
                zz = (edges-mkp)/sigkp; cF = normcdf(zz);
                w  = zeros(npk,1); w(kmin:kmax) = diff(cF)/kap;
                ssum = sum(w);
                if ssum <= 0 || ~isfinite(ssum)
                    c  = min(max(mkp, L), U);
                    iF = Kmap(c);  iLo = max(1,floor(iF));  iHi = min(npk,iLo+1);
                    w  = zeros(npk,1);  w(iLo) = iHi-iF;  w(iHi) = 1-(iHi-iF);
                else
                    w = w / ssum;
                end
    ...
    params.wKp_grid=wKp_grid;





    % probkp_today = zeros(npk,npk*nsigK);
    % 
    % for sigKc=1:nsigK
    %     for kc=1:npk
    %         idc = kc+npk*(sigKc-1);
    %         mkp=logKgrid(kc);
    %         vkp=params.sigKgrid(sigKc);
    %         sigkp=sqrt(vkp);
    %         if(mkp<=L)
    %             iFloat = Kmap(L);                                   
    %             iL = floor(iFloat);                
    %             iU = min(iL+1, npk);                
    %             omegaK = iU - iFloat;              
    %             probkp_today(iL,idc)=omegaK;
    %             probkp_today(iU,idc)=1-omegaK;
    % 
    %         elseif(mkp>=U)
    %             iFloat = Kmap(U);                                   
    %             iL = floor(iFloat);                
    ...
    %                 zz=(edges-mkp)/sigkp;
    %                 cumF = normcdf(zz);
    %                 probkp_today(kmin:kmax,idc)=diff(cumF)/kappaval;
    %                 probkp_today(:,idc) = probkp_today(:,idc)/sum(probkp_today(:,idc));
    %             end
    %         end
    %     end
    % end
    % params.probkp_today=probkp_today;
    ...
    % % % % Need to finish this
    % probkp = zeros(npzp,npk,npk,sigK);
    % kstep = (params.Kgrid(npk)-params.Kgrid(1))/(npk-1);
    % for sigKc = 1:nsigK
    %     sigma_K = params.sigKgrid(sigKc);
    %     for kc = 1:npk
    %         for pzc=1:npzp
    %             for kpc = 1:npk
    %                 if()
    %                 probkp(pzc,kpc,kc,sigKc) = 
    %             end
    %         end
    %     end
    % end
    ...
    save CurrentOutput cpol dec ipol X a0 a1
    % load CurrentOutput.mat
    ...
    if(diff_a>100*tol)
        ...
        dists.sigKdist=dists_out.sigKdist;
...
sigKdist0=dists_out.sigKdist;
ishocksT=shocks.ishocks(:,end);

save([ parIn.saveResultsFile], 'a0', 'a1', 'cpol', 'dec', 'X','ipol', 'kdist0', 'pdist0', 'pKdist0','sigKdist0', 'ishocksT','diff_a','params','Kvec') 
...
save([parIn.saveResultsFile], 'a0', 'a1', 'cpol', 'dec', 'X','ipol', 'kdist0', 'pdist0', 'pKdist0','sigKdist0', 'ishocksT','forecast_moments','moments','diff_a','params','Kvec','Kvec_u','Kvec_e','-v7.3')
end
 end
