...
%Endogenous        =  fullfile(scriptDir, '_aux/_models/model_2044.mat');
%Exogenous         =  fullfile(scriptDir, '_aux/_models/model_2044_exo.mat');
%Fullinformation   =  fullfile(scriptDir, '_aux/_models/model_2044_exo.mat');

Endogenous        =  fullfile(scriptDir, '_aux/_models_tmp/model_2044.mat');
Exogenous         =  fullfile(scriptDir, '_aux/_models_tmp/model_2052.mat');
Fullinformation   =  fullfile(scriptDir, '_aux/_models_tmp/model_2051.mat');
...
% Open the file
fid = fopen(fullfile(scriptDir, '_table/table_cxii.tex'), 'w');
...
tmp1 = [endogenous.ineq(1)  endogenous.ineq(2) endogenous.ineq(3)  endogenous.ineq(4) endogenous.ineq(5)]; 
fprintf(fid, 'Endogenous (A)');
fprintf(fid, ' & %.2f', tmp1);
fprintf(fid, '\\\\ \n');

tmp2 = [exogenous.ineq(1) exogenous.ineq(2)  exogenous.ineq(3)  exogenous.ineq(4) exogenous.ineq(5)]; 
fprintf(fid, 'Exogenous (B)');
fprintf(fid, ' & %.2f', tmp2);
fprintf(fid, '\\\\ \n');

tmp3 = [fullinformation.ineq(1) fullinformation.ineq(2) fullinformation.ineq(3) fullinformation.ineq(4) fullinformation.ineq(5)]; 
fprintf(fid, 'Full Information (C)');
fprintf(fid, ' & %.2f', tmp3);
fprintf(fid, '\\\\ \n');

diff2 = 100*[tmp1(1)/tmp3(1)-1 tmp1(2)/tmp3(2)-1   tmp1(3)/tmp3(3)-1  tmp1(4)/tmp3(4)-1 tmp1(5)/tmp3(5)-1];
fprintf(fid, 'Difference pct. (A vs C)');
